<?php
/**
 * Knowly_Leaderboard_Service — Leaderboard business logic.
 *
 * v2.1 fix:
 ...
 *
 * v2.0:
 *  - Board key: level:period:subject (colon-separated, 'none' for null period)
...
class Knowly_Leaderboard_Service {
     ...
     * @param string   $period    e.g. 'term_1' — pass 'none' for std_5
    ...
    public static function get_board(
        ...
        Knowly_Debug::log( 'leaderboard.fetch', 'Fetching board from Railway', [
            'level'    => $level,
            'period'   => $period,
            'subject'  => $subject,
            'child_id' => $child_id,
        ...
        $period_segment = ( ! $period || $period === 'none' ) ? 'none' : $period;
        $path           = "/api/v1/leaderboard/{$level}/{$period_segment}/{$subject}";
        $response     = self::railway_get( $path, [], $child_id );

        if ( is_wp_error( $response ) ) {
            Knowly_Debug::log( 'leaderboard.fetch', 'Board fetch failed', [
                'path'  => $path,
        ...
        Knowly_Debug::log( 'leaderboard.fetch', 'Board fetched successfully', [
    ...
    public static function get_my_boards( int $child_id ): array|WP_Error {
        Knowly_Debug::log( 'leaderboard.fetch', 'Fetching personal boards', [
            'child_id' => $child_id,
        ], $child_id, 'info' );

        $response = self::railway_get( "/api/v1/leaderboard/me/{$child_id}" );

        if ( is_wp_error( $response ) ) {
            Knowly_Debug::log( 'leaderboard.fetch', 'Personal boards fetch failed', [
                'child_id' => $child_id,
                'error'    => $response->get_error_message(),
            ], $child_id, 'error' );
            return $response;
        ...
        Knowly_Debug::log( 'leaderboard.fetch', 'Personal boards fetched', [
            'child_id'    => $child_id,
    ...
    public static function generate_nickname(
        ...
        Knowly_Debug::log( 'leaderboard.nickname_generate', 'Requesting nickname generation', [
            'child_id' => $child_id,
            'level'    => $level,
            'period'   => $period,
        ], $child_id, 'info' );

        $response = self::railway_post( '/api/v1/leaderboard/generate-nickname', [
        ...
        if ( is_wp_error( $response ) ) {
            Knowly_Debug::log( 'leaderboard.nickname_generate', 'Nickname generation failed', [
                'child_id' => $child_id,
                'error'    => $response->get_error_message(),
            ], $child_id, 'warning' );
            return $response;
        ...
        if ( ! $nickname ) {
            Knowly_Debug::log( 'leaderboard.nickname_generate', 'Railway returned empty nickname', [
                'child_id' => $child_id,
            ], $child_id, 'warning' );
            return new WP_Error( 'knowly_nickname_empty', 'Nickname generation returned empty.', [ 'status' => 500 ] );
        }

        update_user_meta( $child_id, 'knowly_nickname', $nickname );
        delete_user_meta( $child_id, 'knowly_nickname_pending' );

        Knowly_Debug::log( 'leaderboard.nickname_generate', 'Nickname generated and saved', [
            'child_id' => $child_id,
            'nickname' => $nickname,
    ...
    public static function regenerate_nickname(
        ...
        $old_nickname = get_user_meta( $child_id, 'knowly_nickname', true ) ?: 'unknown';

        Knowly_Debug::log( 'leaderboard.admin', 'Nickname regeneration requested', [
            'child_id'     => $child_id,
            'admin_id'     => $admin_id,
        ...
        $response = self::railway_post( '/api/v1/leaderboard/regenerate-nickname', [
            'user_id'  => (string) $child_id,
        ...
        if ( is_wp_error( $response ) ) {
            Knowly_Debug::log( 'leaderboard.admin', 'Nickname regeneration failed', [
                'child_id' => $child_id,
                'error'    => $response->get_error_message(),
            ], $admin_id, 'error' );
            return $response;
        ...
        if ( $new_nickname ) {
            update_user_meta( $child_id, 'knowly_nickname', $new_nickname );
        }

        Knowly_Debug::log( 'leaderboard.admin', 'Nickname regenerated successfully', [
            'child_id'     => $child_id,
            'admin_id'     => $admin_id,
            'old_nickname' => $old_nickname,
     ...
     * Railway expects slugs (e.g. "math"). Knowly_Exam_Service::normalise_subject()
     * handles all known mappings.
     *
     * @param  array $session  Row from knowly_exam_sessions.
    ...
    public static function handle_submit_upsert( array $session, array $result ): ?array {
        ...
        try {
            ...
            $subject_normalised = Knowly_Exam_Service::normalise_subject( $subject_raw );
            // ── END FIX ─────────────────────────────────────────────────────

            Knowly_Debug::log( 'leaderboard.upsert', 'Accumulating leaderboard points', [
                'child_id'           => $child_id,
                'level'              => $session['level'] ?? '',
            ...
            $response = self::railway_post( '/api/v1/leaderboard/upsert', [
                'user_id'         => (string) $child_id,
                'nickname'        => get_user_meta( $child_id, 'knowly_nickname', true ) ?: 'Player' . $child_id,
                'level'           => $session['level'] ?? '',
                'period'          => $session['period'] ?? '' ?: 'none',
            ...
            if ( is_wp_error( $response ) ) {
                Knowly_Debug::log( 'leaderboard.upsert_failed', 'Leaderboard upsert failed', [
                    'child_id' => $child_id,
                    'error'    => $response->get_error_message(),
                ], $child_id, 'error' );
                return null;
            ...
            Knowly_Debug::log( 'leaderboard.upsert', 'Points accumulated successfully', [
                'child_id'     => $child_id,
                'points_added' => $points,
            ...
            Knowly_Debug::log( 'leaderboard.upsert_failed', 'Leaderboard upsert threw exception', [
                'child_id' => $child_id,
     ...
     * Called by Knowly_Cron at 04:00 UTC (Trinidad midnight).
     *
     * @param  int $admin_id  0 = cron/system trigger.
     * @return array|WP_Error
     */
    public static function trigger_daily_reset( int $admin_id = 0 ): array|WP_Error {
        Knowly_Debug::log( 'leaderboard.reset', 'Daily leaderboard reset triggered', [
            'admin_id' => $admin_id,
            'trigger'  => $admin_id > 0 ? 'manual' : 'cron',
            'utc_time' => current_time( 'mysql', true ),
        ], $admin_id ?: null, 'info' );

        $response = self::railway_post( '/api/v1/leaderboard/reset', [
            'admin_id' => (string) $admin_id,
            'trigger'  => $admin_id > 0 ? 'manual' : 'cron',
        ] );

        if ( is_wp_error( $response ) ) {
            Knowly_Debug::log( 'leaderboard.reset', 'Daily reset failed', [
                'error' => $response->get_error_message(),
            ], null, 'error' );
            return $response;
        }

        Knowly_Debug::log( 'leaderboard.reset', 'Daily reset completed', [
    ...
    public static function inject_test_entry( array $data ): array|WP_Error {
        Knowly_Debug::log( 'leaderboard.test', 'Injecting test entry', $data, null, 'info' );

        $response = self::railway_post( '/api/v1/leaderboard/test/inject', $data );

        if ( is_wp_error( $response ) ) {
            Knowly_Debug::log( 'leaderboard.test', 'Test entry injection failed', [
                'error' => $response->get_error_message(),
            ], null, 'error' );
        }
     ...
     * @return array|WP_Error
     */
    public static function reset_board( string $level, string $period, string $subject ): array|WP_Error {
        Knowly_Debug::log( 'leaderboard.test', 'Resetting specific board', [
            'level'   => $level,
            'period'  => $period,
            'subject' => $subject,
        ], null, 'info' );

        return self::railway_post( '/api/v1/leaderboard/test/reset-board', [
            'level'   => $level,
            'period'  => $period ?: null,
            'subject' => $subject,
        ] );
    ...
    private static function parse_railway_response( mixed $response, string $path ): array|WP_Error {
        if ( is_wp_error( $response ) ) {
            Knowly_Debug::log( 'leaderboard.railway', 'Railway HTTP error', [
                'path'  => $path,
                'error' => $response->get_error_message(),
            ], null, 'error' );
            return new WP_Error( 'knowly_railway_error', 'Failed to connect to leaderboard service.', [ 'status' => 503 ] );
        }
        ...
        if ( $code < 200 || $code >= 300 ) {
            Knowly_Debug::log( 'leaderboard.railway', 'Railway bad response', [
                'path'      => $path,
                'http_code' => $code,
                'body'      => $body,
            ], null, 'error' );
            $message = $body['error'] ?? 'Leaderboard service returned an error.';
            return new WP_Error( 'knowly_railway_error', $message, [ 'status' => $code ] );
        }
