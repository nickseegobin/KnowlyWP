 ...
 *     If a product has `_knowly_token_amount > 0`, that amount × quantity is
 *     credited to the purchasing parent's wallet.
 *  3. Order meta `_knowly_tokens_granted` is set after crediting so the hook
...
class Knowly_WooCommerce {
    ...
    public static function render_token_field(): void {
        ...
        woocommerce_wp_text_input( [
            ...
            'value'             => get_post_meta( get_the_ID(), '_knowly_token_amount', true ) ?: '',
        ] );
    ...
    /**
     * Save the `_knowly_token_amount` field when the product is saved.
     */
    public static function save_token_field( int $post_id ): void {
        $amount = (int) ( $_POST['_knowly_token_amount'] ?? 0 );
        if ( $amount > 0 ) {
            update_post_meta( $post_id, '_knowly_token_amount', $amount );
        } else {
            delete_post_meta( $post_id, '_knowly_token_amount' );
        }
    }
    ...
    public static function handle_order_completed( int $order_id ): void {
        ...
        if ( $order->get_meta( '_knowly_tokens_granted' ) ) {
        ...
        if ( ! $wp_user_id ) {
            Knowly_Debug::log( 'woocommerce.order', 'Order has no WP user — cannot credit tokens', [
                'order_id' => $order_id,
            ], null, 'warning' );
            return;
        }
        ...
        if ( ! array_intersect( $roles, [ 'knowly_parent', 'administrator' ] ) ) {
            Knowly_Debug::log( 'woocommerce.order', 'Buyer is not a knowly_parent — skipping token credit', [
                'order_id'   => $order_id,
        ...
        foreach ( $order->get_items() as $item ) {
            ...
            $token_amount = (int) get_post_meta( $product_id, '_knowly_token_amount', true );
        ...
        if ( $total_credited > 0 ) {
            ...
            $order->update_meta_data( '_knowly_tokens_granted', $total_credited );
            $order->add_order_note(
                sprintf( 'Noey: %d token(s) credited to user #%d.', $total_credited, $wp_user_id )
            );
            $order->save();

            Knowly_Debug::log( 'woocommerce.order', 'Tokens credited from WooCommerce order', [
                'order_id'       => $order_id,
                'wp_user_id'     => $wp_user_id,
